...
def getSecondLevelCandidateSet(first_level_patterns):
    ...
    for i in range(0, len(pages)):
        ...
        for j in range(i+1,len(pages)):
            ...
            if cur_or <= max_or and cur_cs  <= max_cs:
                second_level_patterns[tuple([pages[i][0]])].append({
                    'page':pages[j][0],
                    'overlap':cur_or,
                    'coverage':cur_cs
                })
...
def ParallelCmine(inputFile, min_rf, folderPath):
    ...
    cur_level_patterns = getSecondLevelCandidateSet(first_level_patterns)
    
    level = 2
    
    logger.info("saving {} level patterns".format(level))
    save_nth_level_patterns(os.path.join(folderPath,"{}_level_patterns.txt".format(level)), cur_level_patterns)
    level += 1
    
    while True:
        if len(cur_level_patterns) < 1:
            break
        logger.info("computing patterns for level : {}".format(level))
        with Manager() as manager:
            new_patterns = manager.dict()
            with Pool(processes=numOfProcesses) as pool:
                pool.map(getNextLevelCandidateSetMapper, args_yielder(cur_level_patterns, new_patterns))
            cur_level_patterns = dict(new_patterns)
            logger.info("saving {} level patterns".format(level))
            if len(cur_level_patterns) != 0:
                save_nth_level_patterns(os.path.join(folderPath,"{}_level_patterns.txt".format(level)), cur_level_patterns)
            level += 1
