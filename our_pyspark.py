...
def getSecondLevelCandidateSet( max_or, max_cs, transactions_len, first_level_patterns):
    ...
    for i in range(0, len(pages)):
        ...
        for j in range(i+1,len(pages)):
            ...
            cur_cs = union_len/transactions_len
    ...
    if True:
        ...
        for i in range(len(same_part_key_value)):
            ...
            for j in range(i+1,len(same_part_key_value)):
                ...
                cur_or = intersection_len/len(page_to_transactions.value[new_page])
                cur_cs = union_len/transactions_len
                # if level == 4:
...
def args_yielder(cur_level_patterns, transactions_len):
    for same_part_key in cur_level_patterns:
        yield same_part_key, transactions_len, cur_level_patterns[same_part_key]
...
# max_cs = None
    ...
    # print(page_to_transactions.value)
    print(web_page_counters)
    first_level_patterns = Counter({k: c for k, c in web_page_counters.items() if c >= min_rf.value*len(transactions.value) and c <= max_cs.value*len(transactions.value) })
    ...
    if True:
        ...
        # even_level_patterns = getSecondLevelCandidateSet(page_to_transactions, max_or, max_cs, len(transactions.value), first_level_patterns)
        ...
        while True:
            ...
            if True:
                if level%2==0:
                    ...
                    result = pool.map(getNextLevelCandidateSetMapper).collect()
...
if __name__ == "__main__":
    ...
    conf = SparkConf().setAppName(APP_NAME).setAll([('spark.default.parallelism',numOfProcesses)])
    ...
    max_or = sc.broadcast(max_or)
    t1 = time.time()
    output = ParallelCmine(sc, transactions_file_path, min_rf, folderPath, numOfProcesses)
    t2 = time.time()
